<script lang="ts">
    ...
    async function login() {
        const response = await fetch(`${apiUrl}/auth/login-options?email=${email}`, {
            credentials: "include",
        });
        ...
        options.allowCredentials = options.allowCredentials.map(cred => ({
            ...cred,
            id: Uint8Array.from(atob(fixBase64url(cred.id)), c => c.charCodeAt(0))
        }));
...
<Button class="w-36 self-center grid grid-cols-3 px-2.5" on:click={handleClick} kind="secondary">
